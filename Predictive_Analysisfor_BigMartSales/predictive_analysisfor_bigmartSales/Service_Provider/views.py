...
def ViewTrendings(request):
    topic = Bigmart_model.objects.values('topics').annotate(
        dcount=Count('topics')).order_by('-dcount')
    return render(request, 'SProvider/ViewTrendings.html', {'objects': topic})


def charts(request, chart_type):
    chart1 = detection_values_model.objects.values(
        'names').annotate(dcount=Avg('MSE'))
    return render(request, "SProvider/charts.html", {'form': chart1, 'chart_type': chart_type})


def charts1(request, chart_type):
    chart1 = detection_values_model.objects.values(
        'names').annotate(dcount=Avg('RMSE'))
...
def likeschart(request, like_chart):
    charts = detection_values_model.objects.values(
        'names').annotate(dcount=Avg('MAE'))
...
def Download_Trained_DataSets(request):
    ...
    # decide file name
    response['Content-Disposition'] = 'attachment; filename="Predictions_Results.xls"'
    # creating workbook
...
def train_model(request):
    ...
    train = pd.read_csv(
        r"C:\Users\talar\OneDrive - MALLA REDDY GROUP OF INSTITUTIONS\Desktop\Major Project\Predictive_Analysisfor_BigMartSales\predictive_analysisfor_bigmartSales\Train.csv")
    test = pd.read_csv(
        r"C:\Users\talar\OneDrive - MALLA REDDY GROUP OF INSTITUTIONS\Desktop\Major Project\Predictive_Analysisfor_BigMartSales\predictive_analysisfor_bigmartSales\Test.csv")
    test1 = test.copy()
    ...
    feat = ['Outlet_Size', 'Outlet_Type',
            'Outlet_Location_Type', 'Item_Fat_Content', "Item_Type"]
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        df_train, y_train, test_size=0.4)
    ...
    detection_values_model.objects.create(names="Linear Regression Model Results", MAE=metrics.mean_absolute_error(
        y_test, preds), MSE=metrics.mean_squared_error(y_test, preds), RMSE=np.sqrt(metrics.mean_squared_error(y_test, preds)))
